...
export const workFilesApi = {
  ...
  firstByWorkIds: (workIds: number[]) => api.invoke('work-files:firstByWorkIds', workIds) as Promise<{ work_id: number; file_path: string; type: string }[]>,
...
export const cupApi = {
  ...
  headToHead: (type: 'actor' | 'work', itemId: number, seasonId?: number, minMatches?: number) =>
    api.invoke('cup:head-to-head', { type, itemId, seasonId, minMatches }) as Promise<{ opp_id: number; total: number; wins: number; losses: number; draws: number; opp_rank?: number | null; name?: string; title?: string; product_number?: string; photo_path?: string; cover_path?: string }[]>,
}
