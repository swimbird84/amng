...
**조별 예선 전체 완료 → 블럭 토너먼트 순차 진행 → 결승 라운드** 순으로 진행됩니다.
...
블럭은 **UI 분산 목적**입니다. 128명 전체 대진표를 한 화면에 표시하면 가독성이 떨어지기 때문에
블럭 단위로 나눠서 표시합니다. 실제 진행 구조는 **조별리그 → 하나의 연속된 토너먼트**이며,
블럭 경계는 UI에서만 의미가 있습니다.
...
```
1. 조별 예선 (모든 블럭의 모든 조 순차 진행, 기존과 동일)
   ↓ 전체 완료
2. 블럭 A 토너먼트 (32명, 4라운드, 최종 2인 진출)
   ↓ 완료
3. 블럭 B 토너먼트
   ↓ 완료
4. 블럭 C 토너먼트
...
### 핵심 원칙
- **블럭 1개 = 조 16개, 진출자 32명 (2명 × 16조)**
- `blockCount = groupCount / 16 = round_total / 32`
  - 예: round_total = 64  → groupCount=32  → 2블럭 (A, B)
  - 예: round_total = 128 → groupCount=64  → 4블럭 (A, B, C, D)
  - 예: round_total = 256 → groupCount=128 → 8블럭 (A~H)

### 블럭 토너먼트 진행
- 조별 예선 **전체** 완료 후 블럭 A부터 순서대로 시작
- 블럭 A 완료 → 블럭 B 시작, 블럭 B 완료 → 블럭 C 시작, ...
- 각 블럭 소속 16개 조의 진출자(조당 2명) 32명 수집 후 랜덤 셔플
- **4라운드 진행 후 2명 생존** → 결승 라운드로 진출 (블럭 내 결승 없음)
...
블럭 토너먼트에서 이어지는 형태이므로 별도 구분 없이 라운드명만 표기 (n인 없음)

| 블럭 수 | 결승 라운드 진출 인원 | 라운드 구성          |
|--------|---------------------|---------------------|
| 2블럭  | 4명                 | 준결승 → 결승        |
| 4블럭  | 8명                 | 8강 → 준결승 → 결승  |
| 8블럭  | 16명                | 16강 → 준결승 → 결승 |
...
조별/동점처리 경기 완료 시:
  ...
  - 완료 시 → 블럭 A(block_id=0)의 32명 수집 → 블럭 A 1라운드 생성

블럭 본선 경기(phase='main', block_id=N) 완료 시:
  - 해당 block_id 기준 라운드 완료 체크
  - 남은 인원 > 2명 → 다음 라운드 매치 생성 (block_id 유지)
  - 남은 인원 = 2명 (4라운드 완료):
    - 다음 블럭이 있으면 → startNextBlock(runId, N+1)
    - 마지막 블럭이면 → 전 블럭 진출자 수집 → 결승 라운드 1라운드 생성 (block_id=NULL)
...
### 현황 탭 — 조별 예선 중
- 블럭 레이블로 섹션 구분 (블럭 A, 블럭 B, ...)
- 각 블럭 내 16개 조 카드 그리드 (기존 조 카드 UI 재사용)
- 현재 진행 중인 조 카드가 해당 블럭 내에서 맨 앞으로

### 현황 탭 — 블럭 토너먼트 / 결승 라운드 중

각 블럭 = 카드 1개, 진행 중인 블럭이 최상단
...
결승 라운드는 별도 섹션으로 블럭 카드 아래에 표시 (블럭 전체 완료 후 활성화)
...
6. `ipc.ts`: `cup:pick` worldcup — 블럭 순차 진행 + 결승 라운드 연결
7. `ipc.ts`: `cup:standings` worldcup — 블럭별 + 결승 라운드 구조 반환

### Phase C: UI
8. `Worldcup.tsx` 현황 탭 worldcup — 블럭 카드 대진표 + 결승 라운드 섹션
...
- [x] 크로스 블럭 토너먼트: 설계에서 제외
- [x] 블럭 구조 본질: UI 분산 목적, 실제는 연속된 하나의 토너먼트
- [ ] 블럭 토너먼트 4라운드 완료 시 정확히 2인 보장 로직 확인 (32명 → 4라운드 → 2명: 32→16→8→4→2 ✓)
