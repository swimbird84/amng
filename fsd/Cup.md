...
| `cupApi.divisionCounts(type)` | `cup:division-counts` | 부별 인원 수 |
...
| `cupApi.headToHead(type, itemId)` | `cup:head-to-head` | 상대전적 |
...
| API 함수 | IPC 채널 | 설명 |
|----------|----------|------|
| `masterRankingApi.list(params)` | `master-ranking:list` | 마스터 랭킹 목록 |
| `masterRankingApi.reset(type)` | `master-ranking:reset` | 마스터 포인트 리셋 |
| `masterRankingApi.rankTrends(type)` | `master-ranking:rank-trends` | 순위 변동 |
| `masterRankingApi.itemFormatStats(type, itemId)` | `master-ranking:item-format-stats` | 포맷별 전적 |
| `masterRankingApi.rankHistory(type, itemId)` | `master-ranking:rank-history` | 순위 이력 |
| `masterRankingApi.divisionHistory(type, itemId)` | `master-ranking:division-history` | 부별 이력 |
| `masterRankingApi.itemStats(type, itemId)` | `master-ranking:item-stats` | 아이템 종합 통계 |
...
| `cup_runs` | 대회 회차 (tournament_id, round_total, status, started_at, completed_at) |
...
| `master_ranking` | 마스터 랭킹 포인트 (type, item_id, total_points, last_updated) |
| `master_ranking_history` | 마스터 랭킹 이력 (type, item_id, rank, total_points, recorded_at) |
